...
class RepairZip(ZipFile):
    ...
    def __init__(self, filename, mode="r", compression=ZIP_STORED, allowZip64=True, compress_level=None, strict=True):
        """
        Open the ZIP file with mode read "r", write "w" or append "a".
        :param filename: The corrupted file
        :param mode: The mode to open the file("r", "w", "a")
        :param compression: The compression method
        :param allowZip64: indicating whether to use zip64 extensions
        :param compress_level: compression level(relevant only if compression = ZIP_DEFLATED)
        :param strict: indicate whether to raise errors or handle them silently
        """
        ...
        self.file_list = []  # List of ZipInfo instances for archive
        ...
        try:
            if key == "r":
                self.fp.seek(0)
                if stringFileHeader not in self.fp.read(1024):  # check if stringFileHeader is in the header of the file
                    self.is_zip = False
                self._RealGetContents()  # Read in the table of contents for the ZIP file
            elif key == "w":
                # set the modified flag so central directory gets written
                # even if no files are added to the archive
                self._didModify = True
                try:
                    self.start_dir = self.fp.tell()  # the current place in the file
    ...
    def fix_zip(self):
        ...
        if not self.broken:
            ...
            return False
        self.fp.seek(0, 2)  # go to the end of the file
        file_len = self.fp.tell()
        mm = mmap.mmap(self.fp.fileno(), 0, access=mmap.ACCESS_READ)  # creates a memory-mapped file object(read only).
        offset = 0
        file_list = {}
        cd_list = {}
        ...
        try:
            ...
            while offset + 4 < file_len:
                hdr_off = mm.find(b"PK", offset)  # looking for headers
                if hdr_off == -1:  # didn't find header
                    break
                hdr_type = mm[hdr_off: hdr_off + 4]  # finding the header's type
                if hdr_type == stringFileHeader:
                    # local file header
                    if hdr_off + sizeFileHeader > file_len:  # checks if the header fits within the size length
                        break
                    ...
                    offset = hdr_off + sizeEndCentDir  # skip to the next file
                else:
                    offset = hdr_off + 1
            ...
            for filename, (start, end, centdir) in cd_list.items():
                # go over all the files with stringCentralDir header
                ...
                # change arguments in the ZipInfo object with centdir and mm
                ...
                try:
                    x._decodeExtra()  # try to decode additional metadata for 'x'.
                except TypeError:
            ...
            for filename, (start, end, fheader) in file_list.items():
                # go over all the files inside file_list that are not inside cd_list
                # (everything except the files that were in the previous for loop)
                if filename in cd_list:
                    continue

                x = ZipInfo(filename.decode("utf-8", "backslashreplace"))
                # change attribute in ZipInfo object based on the guess values
                x.extra = ""
                x.comment = ""
