...
class MongoDatabaseAdapter(DatabaseAdapter):
    def __init__(self,  logger, mongo_address, mongo_port, mongo_username, mongo_password,
                 server_timeout=4000, keep_connection=True):
    ...
    def is_collection(self, db_name, collection_name):
        ...
        """
        # check_collection
        if collection_name in self._client_connection[db_name].list_collection_names():
            return True
        return False
    ...
    def is_db(self, db_name):
        ...
        :return: True/False
        """
        if db_name in self._client_connection.list_database_names():
            return True
        return False
